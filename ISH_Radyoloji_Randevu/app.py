...
class Islem(db.Model):
    ...
    notlar = db.Column(db.Text, nullable=True) # YENİ NOTLAR ALANI
    def to_dict(self):
...
db.init_app(app)
# ... (login_manager ve rol kontrolü aynı kalıyor) ...
...
with app.app_context():
    ...
    if User.query.count() == 0:
        ...
        db.session.commit()
        print("Varsaylan admin kullanıcısı oluşturuldu: admin / admin123")

# ... (login, logout, ana_sayfa rotaları aynı kalıyor) ...
...
def islem_ekle():
    ...
    yeni_islem = Islem(
        ...
        notlar=request.form.get('notlar') # YENİ: Notları formdan al
    )
...
# --- YENİ DÜZENLEME VE SİLME ROTALARI ---
...
def duzenle(islem_id):
    ...
    if request.method == 'POST':
        # Formdan gelen güncel bilgileri al ve veritabanını güncelle
    ...
    # GET request için, formu mevcut bilgilerle dolu olarak göster
...
# ... (Diğer tüm rotalarınız aynı kalıyor: /yeni, /gun, /arama, /admin, api vs.)
#... (Önceki mesajdaki diğer tüm rotalarınız buraya kopyalanacak)
@app.route('/yeni')
@login_required
@role_required(['admin', 'doktor'])
